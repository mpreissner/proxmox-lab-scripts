...
- **Mistral Le Chat traffic generation** — a new `mistral-prompt.py` helper is installed to `/opt/traffic-gen/utils/`. It launches headless Chromium with `--disable-blink-features=AutomationControlled` and overrides `navigator.webdriver` to pass Cloudflare bot detection, accepts the terms modal (`"Accept and continue"`), and submits prompts via `button[aria-label="Send question"]`. `--disable-quic` is added to both `mistral-prompt.py` and `gemini-prompt.py` to force TCP so ZIA can perform TLS inspection (QUIC/HTTP3 over UDP bypasses it). Mistral is enabled as a `genai_provider` for the `devops` and `developer` profiles and included in the `dlp-genai-prompt.sh` platform rotation. **Note:** ZIA prompt capture for Mistral requires Client Connector (ZCC) due to TLS fingerprint-based app classification; containers connecting via IPSec tunnel will generate allowed/blocked Mistral traffic and DLP events but not prompt capture events.

### Changed
- **Container memory** — all genai-enabled profile containers (`office-worker`, `devops`, `developer`, `sales`, `executive`) bumped from 512 MB to 768 MB. Cloudflare's bot challenge JS requires significantly more memory than Gemini's page, causing page crashes at 512 MB when loading Mistral Le Chat.
...
### Changed
- **Container memory** — `office-worker`, `sales`, and `executive` profile containers bumped from 256 MB to 512 MB to accommodate headless Chromium. `developer`, `devops`, and `monitoring` were already at 512 MB. All profiles that invoke headless Chromium (via `genai_web_prompt` or `dlp-genai-prompt`) are now at 512 MB minimum. (Superseded by 3.6.1 which bumps all genai-enabled profiles to 768 MB.)
...
### Fixed
- **`cmd_stop_containers` and `cmd_start_containers` Data Center / Branch scope** — options 2 and 3 in both functions previously iterated every integer in the configured CTID range via `seq`, which included VMs and untagged containers sharing that range. They now filter the already tag-scoped `ALL_RUNNING` / `STOPPED_CONTAINERS` lists by range membership, so only `lab-managed` LXC containers are ever targeted.
